...
```{r}
daily_shelter_usage_overall <- shelter_usage %>%
  select(OCCUPANCY_DATE, SECTOR, OCCUPANCY, PROGRAM_NAME) %>% 
  mutate(DATE = substr(OCCUPANCY_DATE, 1, 10)) %>% 
  group_by(DATE) %>% 
  summarise(OCCUPANCY = sum(OCCUPANCY))

plot1 <- daily_shelter_usage_overall %>% 
  ggplot(aes( y = OCCUPANCY, x = DATE, color="Overall")) +
  geom_point() +
  labs(title = "2020 Daily Shelter Usage by Sector", y = "Daily Occupancy", x = "Date", subtitle = "2020-01-01 to 2020-09-26", color="")+
  theme_minimal() 
...
```{r}
monthly_shelter_usage <- shelter_usage %>%
  select(OCCUPANCY_DATE, SECTOR, OCCUPANCY, PROGRAM_NAME) %>% 
  mutate(MONTH = substr(OCCUPANCY_DATE, 6, 7)) %>% 
  group_by(MONTH) %>% 
  summarise(OCCUPANCY = sum(OCCUPANCY))
...
```{r}
number_of_unique_shelters <- shelter_usage %>%
  mutate(DATE = substr(OCCUPANCY_DATE, 1, 10), UNIQUE_sHELTERS = 0) %>% 
  group_by(DATE) %>% 
  summarise(UNIQUE_SHELTERS = n_distinct(PROGRAM_NAME))

plot3 <- number_of_unique_shelters %>% 
  ggplot(aes( y = UNIQUE_SHELTERS, x = DATE, color = "Overall")) +
  geom_point() +
...
```{r}
number_of_unique_shelters_monthly <- shelter_usage %>%
  select(OCCUPANCY_DATE, SECTOR, OCCUPANCY, PROGRAM_NAME) %>% 
  mutate(MONTH = substr(OCCUPANCY_DATE, 6, 7), UNIQUE_SHELTERS = 0) %>% 
  group_by(MONTH) %>% 
  summarise(UNIQUE_SHELTERS = n_distinct(PROGRAM_NAME))

number_of_unique_shelters_monthly
```
...
```{r}
daily_shelter_usage <- shelter_usage %>%
  select(OCCUPANCY_DATE, SECTOR, OCCUPANCY, PROGRAM_NAME) %>% 
  mutate(DATE = substr(OCCUPANCY_DATE, 1, 10)) %>% 
  group_by(DATE, SECTOR) %>% 
  summarise(OCCUPANCY = sum(OCCUPANCY))

plot2 <- daily_shelter_usage %>% 
  ggplot(aes( y = OCCUPANCY, x = DATE, color = SECTOR)) +
  geom_point() +
